...
## [2.0.9] — 2026-08-29（[TASK-V208-UNBIND-FIX]，修复 Web 解绑后策略仍下发 + 未绑定设备管控残留）

### 修复
- **匿名注册不再创建默认策略**：`POST /api/v1/device/register` 新建设备时不再预建 120 分钟策略；
  策略改在绑定成功时补建（`/api/v1/device/bind-with-code` 与 `/api/pairing/verify`）。
  修复 Web 解绑（硬删除设备行）后儿童端匿名重注册即“自动下发”默认策略的根因。
- **未绑定设备不返回策略/公告/下行指令**：`GET /api/v1/device/policies` 对无归属设备返回空策略；
  `POST /api/v1/device/heartbeat` 对无归属设备返回 `policyVersion=0`、空公告签名、空指令。
  客户端据此清空本地策略缓存，解绑后管控随即失效。

### 单测
- 全量 343/343 通过（新增：匿名注册不建策略 / 未绑定设备策略为空 / 未绑定心跳零版本 /
  绑定补建默认策略 / 绑定归属与配对状态断言）
